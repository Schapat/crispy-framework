...
| US-004 | Governance intake TUI writes project-start markdown artifacts without app scaffolding | yes | yes | no | yes | implemented | `scripts/test-governance-intake.sh`, `bash -n scripts/governance-intake`, `bash -n scripts/test-governance-intake.sh`, PTY Enter/Ctrl-C smoke test |
...
| US-010 | Installer creates missing harness files and never overwrites target files | no | yes | no | yes | implemented | `docs/decisions/0013-non-overwriting-installer.md`, installer syntax and existing-target probes |
