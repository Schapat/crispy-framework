...
This file describes the repository-local Harness CLI. It records operational
harness state; it does not define product behavior or replace the markdown
governance docs.
...
Use `docs/WORK_INTAKE.md` for the accepted input-type definitions. The CLI
normalizes those types into durable records.
...
| `intake` | Task specification | Record a work intake classification. | `--type`, `--summary`, `--lane` |
...
- Responsibilities must match the Crisp Harness component responsibility list.
