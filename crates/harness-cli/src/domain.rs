...
pub enum ParseHarnessValueError {
    #[error("unknown intake type '{0}'. Use a type from docs/WORK_INTAKE.md, such as new spec, change request, existing-project adoption, prototype, resume request, or harness improvement")]
    InputType(String),
...
pub enum InputType {
    ...
    NewInitiative,
    Maintenance,
    ExistingProjectAdoption,
    FirstExistingProjectAdoption,
    HarnessImprovement,
    Prototype,
    ResumeRequest,
}
...
impl InputType {
    pub fn as_db_value(&self) -> &'static str {
        match self {
            ...
            Self::ExistingProjectAdoption => "existing_project_adoption",
            Self::FirstExistingProjectAdoption => "first_existing_project_adoption",
            Self::HarnessImprovement => "harness_improvement",
            Self::Prototype => "prototype",
            Self::ResumeRequest => "resume_request",
        }
    }
}
...
impl FromStr for InputType {
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        ...
        match normalized.as_str() {
            ...
            "existing_project_adoption" => Ok(Self::ExistingProjectAdoption),
            "first_existing_project_adoption" => Ok(Self::FirstExistingProjectAdoption),
            "harness_improvement" => Ok(Self::HarnessImprovement),
            "prototype" | "spike" => Ok(Self::Prototype),
            "resume_request" => Ok(Self::ResumeRequest),
...
pub fn compiled_tool_registry() -> Vec<ToolEntry> {
    vec![
        ...
        tool(
            ...
            "intake",
            "intake",
            "Record a work intake classification.",
            &[
                ("type", "string", true),
                ("summary", "string", true),
                ("lane", "enum", true),
...
fn add_base_context_rules<'a>(
    ...
    match phase {
        ...
        _ => {
            ...
            must.push(("Work intake", "docs/WORK_INTAKE.md"));
            must.push(("Durable matrix", "scripts/bin/harness-cli query matrix"));
            if lane == "tiny" {
...
mod tests {
    ...
    fn parses_input_type_aliases() {
        ...
        assert_eq!(
            "existing-project adoption".parse::<InputType>().unwrap(),
            InputType::ExistingProjectAdoption
        );
        assert_eq!(
            "first existing project adoption"
                .parse::<InputType>()
                .unwrap(),
            InputType::FirstExistingProjectAdoption
        );
        assert_eq!("spike".parse::<InputType>().unwrap(), InputType::Prototype);
        assert_eq!(
            "resume request".parse::<InputType>().unwrap(),
            InputType::ResumeRequest
        );
    }
